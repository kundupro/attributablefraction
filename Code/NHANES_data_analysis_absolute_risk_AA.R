...
#---18+
nhanes_combined = nhanes_combined[which(nhanes_combined$RIDAGEYR >= 18 & nhanes_combined$RIDAGEYR < 85), ]
...
for(j in 1:length(bmi_cat_names))
{
  ...
  fit_full_obese = svyglm(mortstat ~ bmi_obese + sex + age + education + smoking_status + no_of_cigarettes + alcohol_status_new + marital_status, design=nhanes_svydesign, family = quasibinomial())
  fit_full_obese_severe = svyglm(mortstat ~ bmi_severe_obese + sex + age + education + smoking_status + no_of_cigarettes + alcohol_status_new + marital_status, design=nhanes_svydesign, family = quasibinomial())
...
p1_all_AA = ggplot(data_plot.m, aes(Decile, value)) +   
...
for(j in 1:length(bmi_cat_names))
{
  ...
  for(i in 1:10)
  {
    ...
    print(i)
    AR_fully_adjusted_observed_model_with_e0[j,i] = wtd.mean(exp(as.matrix(temp_2) %*% as.numeric(fit_full$coefficients))/(1 + exp(as.matrix(temp_2) %*% as.numeric(fit_full$coefficients))), temp$sampweight)
    AR_fully_adjusted_observed_model_with_e0_obese[j,i] = wtd.mean(exp(as.matrix(temp3) %*% as.numeric(fit_full_obese$coefficients))/(1 + exp(as.matrix(temp3) %*% as.numeric(fit_full_obese$coefficients))), temp$sampweight)
    AR_fully_adjusted_observed_model_with_e0_obese_severe[j,i] = wtd.mean(exp(as.matrix(temp4) %*% as.numeric(fit_full_obese_severe$coefficients))/(1 + exp(as.matrix(temp4) %*% as.numeric(fit_full_obese_severe$coefficients))), temp$sampweight)
    print(i)
    AR_C_fully_adjusted[j,i] = (prop_exposure_deciles[i,1] + sum(prop_exposure_deciles[i, -1] * exp(fit_full$coefficients[2:6] - MR_estimates_UK_matrix[j, ])))
    AR_C_fully_adjusted[j,i] = AR_C_fully_adjusted[j,i] * AR_fully_adjusted_observed_model_with_e0[j,i]
    print(i)
...
ggsave('/Users/prosenjitkundu/Dropbox/Attributable_risk/Absolute_and_attributable_risk/Plots/absolute_risk_NHANES.png', p1, width = 17,height=11, dpi = 300)
