...
final_data = final_data %>% filter(bmi <= 60) %>% filter(smoking_status == "Never")

final_data = final_data %>% mutate(bmi_cat = case_when(bmi < BMI_quantiles[3] ~ "Underweight",
...
final_data = final_data[-which(final_data$bmi_cat == "Underweight"), ]
...
nhanes_svydesign <- svydesign(data=final_data, id=~SDMVPSU, strata=~SDMVSTRA, weights=~MEC8YR, nest=TRUE)
