...
nhis_AA_2020 = nhis_2020[which(nhis_2020$raceallp_a == 2), ]
...
nhis_AA_2020 = nhis_AA_2020 %>% mutate(bmi_cat = case_when(bmi < BMI_quantiles[3] ~ "Underweight",
                                                                   bmi >= BMI_quantiles[3] & bmi < BMI_quantiles[6] ~ "Normal",
                                                                   bmi >= BMI_quantiles[6] & bmi < BMI_quantiles[8] ~ "ObeseI",
                                                                   bmi >= BMI_quantiles[8] & bmi < BMI_quantiles[10] ~ "ObeseII",
                                                                   bmi >= BMI_quantiles[10] & bmi < BMI_quantiles[12] ~ "ObeseIII",
...
nhis_AA_2020 = nhis_AA_2020[-which(nhis_AA_2020$bmi_cat == "Underweight"), ]
...
prop_exposure = wpct(nhis_AA_2020$bmi_cat, weight = nhis_AA_2020$wtfa_a)
...
#Smokers
nhis_AA_2020 = nhis_2020[which(nhis_2020$raceallp_a == 2), ]
nhis_AA_2020_smokers = nhis_AA_2020[which(as.character(nhis_AA_2020$smoking_status) == "Former" | as.character(nhis_AA_2020$smoking_status) == "Current"), ]
...
nhis_AA_2020_smokers = nhis_AA_2020_smokers %>% mutate(bmi_cat = case_when(bmi < BMI_quantiles[3] ~ "Underweight",
                                                                                   bmi >= BMI_quantiles[3] & bmi < BMI_quantiles[6] ~ "Normal",
                                                                                   bmi >= BMI_quantiles[6] & bmi < BMI_quantiles[8] ~ "ObeseI",
                                                                                   bmi >= BMI_quantiles[8] & bmi < BMI_quantiles[10] ~ "ObeseII",
                                                                                   bmi >= BMI_quantiles[10] & bmi < BMI_quantiles[12] ~ "ObeseIII",
...
#---Removing underweight people--#
nhis_ipums_2000_2005 = nhis_ipums_2000_2005[-which(nhis_ipums_2000_2005$bmi_cat == "Underweight"), ]
nhis_AA_2020_smokers = nhis_AA_2020_smokers[-which(nhis_AA_2020_smokers$bmi_cat == "Underweight"), ]
...
prop_exposure = wpct(nhis_AA_2020_smokers$bmi_cat, weight = nhis_AA_2020_smokers$wtfa_a)
#PAR_MR = 1 - (prop_exposure[1] + sum(prop_exposure[-1] * exp(fit_unadjusted$coefficients[2:6] - MR_estimates_UK))) * (p_obs_D_given_E_equalto_0/p_hat)
PAR_MR = 1 - ((prop_exposure[1] + sum(prop_exposure[-1] * exp(fit_unadjusted$coefficients[2:6] - MR_estimates_UK)))/((prop_exposure[1] + sum(prop_exposure[-1] * exp(fit_unadjusted$coefficients[2:6]))))) 
...
#--Non-smokers#
nhis_AA_2020 = nhis_2020[which(nhis_2020$raceallp_a == 2), ]
nhis_AA_2020_nonsmokers = nhis_AA_2020[which(as.character(nhis_AA_2020$smoking_status) == "Never"), ]
...
nhis_AA_2020_nonsmokers = nhis_AA_2020_nonsmokers %>% mutate(bmi_cat = case_when(bmi < BMI_quantiles[3] ~ "Underweight",
                                                                                         bmi >= BMI_quantiles[3] & bmi < BMI_quantiles[6] ~ "Normal",
                                                                                         bmi >= BMI_quantiles[6] & bmi < BMI_quantiles[8] ~ "ObeseI",
                                                                                         bmi >= BMI_quantiles[8] & bmi < BMI_quantiles[10] ~ "ObeseII",
                                                                                         bmi >= BMI_quantiles[10] & bmi < BMI_quantiles[12] ~ "ObeseIII",
...
#---Removing underweight people--#
nhis_ipums_2000_2005 = nhis_ipums_2000_2005[-which(nhis_ipums_2000_2005$bmi_cat == "Underweight"), ]
nhis_AA_2020_nonsmokers = nhis_AA_2020_nonsmokers[-which(nhis_AA_2020_nonsmokers$bmi_cat == "Underweight"), ]
...
prop_exposure = wpct(nhis_AA_2020_nonsmokers$bmi_cat, weight = nhis_AA_2020_nonsmokers$wtfa_a)
#PAR_MR = 1 - (prop_exposure[1] + sum(prop_exposure[-1] * exp(fit_unadjusted$coefficients[2:6] - MR_estimates_UK))) * (p_obs_D_given_E_equalto_0/p_hat)
PAR_MR = 1 - ((prop_exposure[1] + sum(prop_exposure[-1] * exp(fit_unadjusted$coefficients[2:6] - MR_estimates_UK)))/((prop_exposure[1] + sum(prop_exposure[-1] * exp(fit_unadjusted$coefficients[2:6]))))) 
