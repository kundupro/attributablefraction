...
NHANES_2017 = NHANES_2017 %>% mutate(race_ethnicity = case_when(RIDRETH1 <= 2  ~ "hispanic",
                                                                ...
                                                                RIDRETH1 == 4 ~ "black",
                                                                RIDRETH1 == 6 ~ "asian"))
nhanes_white_2017 = NHANES_2017[which(NHANES_2017$race_ethnicity == "white"),]
#---18+
...
nhanes_combined_white_2017 = nhanes_white_2017
...
nhanes_combined_white_2017 = nhanes_combined_white_2017 %>% mutate(bmi_cat = case_when(bmi < BMI_quantiles[3] ~ "Underweight",
                                                                             bmi >= BMI_quantiles[3] & bmi < BMI_quantiles[6] ~ "Normal",
                                                                             bmi >= BMI_quantiles[6] & bmi < BMI_quantiles[10] ~ "Overweight",
                                                                             bmi >= BMI_quantiles[10] & bmi < BMI_quantiles[12] ~ "SObese_grade1",
                                                                             bmi >= BMI_quantiles[12] & bmi < BMI_quantiles[13] ~ "SObese_grade2",
...
nhanes_combined_white_2017 = nhanes_combined_white_2017[-which(nhanes_combined_white_2017$bmi_cat == "Underweight"), ]


nhanes_combined_white_2017$bmi_shift = NA
nhanes_combined_white_2017$bmi_shift[which(nhanes_combined_white_2017$bmi_cat == "Normal")] = nhanes_combined_white_2017$bmi[which(nhanes_combined_white_2017$bmi_cat == "Normal")]
nhanes_combined_white_2017$bmi_shift[which(nhanes_combined_white_2017$bmi_cat != "Normal")] = mid_point_normal + 0.5*(nhanes_combined_white_2017$bmi[which(nhanes_combined_white_2017$bmi_cat != "Normal")] - mid_point_normal)
nhanes_combined_white_2017 = nhanes_combined_white_2017 %>% mutate(bmi_cat_shift = case_when(bmi_shift < BMI_quantiles[3] ~ "Underweight",
...
#nhanes_white_2017$bmi_cat_shift[which(nhanes_white_2017$bmi_cat_shift == "Underweight")] = "Normal"
nhanes_combined_white_2017 = nhanes_combined_white_2017 %>% filter(bmi_cat == "Normal" | bmi_cat == "Overweight" | bmi_cat == "SObese_grade1" | bmi_cat == "SObese_grade2" | bmi_cat =="SObese_grade3")
...
colnames(nhanes_combined_white_2017)[41] = "sampweight"
nhanes_combined_white_temp_2017 = nhanes_combined_white_2017
nhanes_combined_white_temp_2017$bmi_cat = as.factor(as.character(nhanes_combined_white_temp_2017$bmi_cat))
hh_2017 = model.matrix(~ bmi_cat, data = nhanes_combined_white_temp_2017)
...
hh_2017 = cbind(hh_2017, nhanes_combined_white_temp_2017$sampweight[which(is.na(nhanes_combined_white_temp_2017$bmi_cat) != T)])
hh_2017 = as.data.frame(hh_2017)
colnames(hh_2017)[6] = "sampweight"
hh_2017 = hh_2017 %>% mutate(bmi_cat = as.character(nhanes_combined_white_temp_2017$bmi_cat[which(is.na(nhanes_combined_white_temp_2017$bmi_cat) != T)]), bmi_cat_shift = as.character(nhanes_combined_white_temp_2017$bmi_cat_shift[which(is.na(nhanes_combined_white_temp_2017$bmi_cat) != T)]))



prop_exposure_projected = questionr::wtd.table(hh_2017$bmi_cat, weights = hh_2017$sampweight)/sum(hh_2017$sampweight)
p_projected = sum(baseline_prob*prop_exposure_projected)
...
prop_exposure_projected = as.numeric(prop_exposure_projected)
p_hat = sum(prop_exposure_projected*exp(c(fit_unadjusted$coefficients[1], fit_unadjusted$coefficients[1]+fit_unadjusted$coefficients[-1])))
PAR_percent_shift_se = function(unadjusted_RR, MR_RR, p_hat, transition_matrix, Omega)
{
  transition_matrix[upper.tri(transition_matrix)]  = 0
  k = 1/prop_exposure_projected[1]
  data_processed = nhanes_combined_white_2017 %>% dplyr::select("bmi_cat", "bmi", "sampweight")
  #colnames(data_processed)[3] = "sampweight"
  Q = -matrix(rep(-k, 16), 4, 4) + diag(-1/prop_exposure_projected[-1])
  ...
  psi = t(t(data_processed_mm[, c(2:5)])/prop_exposure_projected[-1]) - ((1-data_processed_mm[,2]-data_processed_mm[,3]-data_processed_mm[,4]-data_processed_mm[,5])/prop_exposure_projected[1])
  
  Delta = (solve(Q) %*% ((t(psi) %*% (psi*data_processed_mm$sampweight))/total_weights) %*% solve(Q))/nrow(data_processed_mm)
  
  Sigma_11 = vcov(fit_unadjusted)
  ...
  #----Construction of beta_derivative vector
  ...
  print(t(g_vector)%*% Sigma_11 %*% g_vector)
  print(t(p_derivative_vector) %*% Delta %*% p_derivative_vector)
  print(t(beta_derivative_vector)%*%Omega%*%beta_derivative_vector)
  
  return(sqrt(t(g_vector)%*% Sigma_11 %*% g_vector + t(p_derivative_vector) %*% Delta %*% p_derivative_vector + t(beta_derivative_vector)%*%Omega%*%beta_derivative_vector))
}

PAR_percent_shift(prop.table(table(hh_2017$bmi_cat, hh_2017$bmi_cat_shift)), baseline_prob, RR, MR_RR, p_projected)

PAR_percent_shift(prop.table(table(hh_2017$bmi_cat, hh_2017$bmi_cat_shift)), baseline_prob, RR, MR_RR, p_projected) - 1.96*PAR_percent_shift_se(fit_unadjusted$coefficients, MR_estimates_UK, p_hat, prop.table(table(hh_2017$bmi_cat, hh_2017$bmi_cat_shift)), Omega = Omega)
PAR_percent_shift(prop.table(table(hh_2017$bmi_cat, hh_2017$bmi_cat_shift)), baseline_prob, RR, MR_RR, p_projected) + 1.96*PAR_percent_shift_se(fit_unadjusted$coefficients, MR_estimates_UK, p_hat, prop.table(table(hh_2017$bmi_cat, hh_2017$bmi_cat_shift)), Omega = Omega)
...
nhanes_combined_white_2017 = nhanes_combined_white_2017[which(nhanes_combined_white_2017$smoking_status == "Former" | nhanes_combined_white_2017$smoking_status == "Current" ),]
...
nhanes_combined_white_2017 = nhanes_combined_white_2017[which(nhanes_combined_white_2017$smoking_status == "Never"),]
