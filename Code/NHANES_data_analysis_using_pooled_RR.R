...
#pooled_estimates = log(c(1.06, 1.19, 1.45, 1.96, 2.89))
pooled_estimates = readRDS("beta_all_cause_meta_pooled.rds")
...
var_beta_all_cause_meta = readRDS("var_beta_all_cause_meta.rds")
#adjustment_factor = (diag(Sigma[2:6,2:6])/var_beta_all_cause_meta)[5]
adjustment_factor = max(diag(Sigma[2:6,2:6])/var_beta_all_cause_meta)
...
#Sigma_33 = t(as.matrix(data_processed_mm[, 2:6])) %*% (as.matrix(data_processed_mm[, 2:6])*data_processed_mm$sampweight)
...
#external variance for pooled estimates
...
delta_HR = exp(MR_vcov_file$est)
MR_estimates_UK = log(c(delta_HR[6] + (delta_HR[8]-delta_HR[6])/2, delta_HR[8] + (delta_HR[10]-delta_HR[8])/2, delta_HR[10] + (delta_HR[12]-delta_HR[10])/2, delta_HR[12] + (delta_HR[13]-delta_HR[12])/2, delta_HR[13] + (delta_HR[14]-delta_HR[13])/2))
#pooled_estimates = log(c(1.12, 1.34, 1.72, 2.67, 3.71))
pooled_estimates = readRDS("beta_cardio_meta_pooled.rds")
...
var_beta_all_cause_meta = readRDS("var_cardio_meta_pooled.rds")
adjustment_factor = max(diag(Sigma[2:6,2:6])/var_beta_all_cause_meta)
var_external_omega = Sigma[2:6,2:6]/adjustment_factor
diag(var_external_omega) = var_beta_all_cause_meta
A = 1 + sum((exp(fit_unadjusted$coefficients[2:6]) - 1)*prop_exposure[-1])
...
MR_estimates_UK = log(c(delta_HR[6] + (delta_HR[8]-delta_HR[6])/2, delta_HR[8] + (delta_HR[10]-delta_HR[8])/2, delta_HR[10] + (delta_HR[12]-delta_HR[10])/2, delta_HR[12] + (delta_HR[13]-delta_HR[12])/2, delta_HR[13] + (delta_HR[14]-delta_HR[13])/2))
#heart_data = nhanes_combined_white
#pooled_estimates = log(c(1.04, 1.12, 1.28, 1.56, 1.86))
pooled_estimates = readRDS("beta_cancer_meta_pooled.rds")
nhanes_combined_white = final_data
nhanes_combined_white = nhanes_combined_white %>% mutate(bmi_cat = case_when(bmi < BMI_quantiles[3] ~ "Underweight",
                                                                             bmi >= BMI_quantiles[3] & bmi < BMI_quantiles[6] ~ "Normal",
                                                                             bmi >= BMI_quantiles[6] & bmi < BMI_quantiles[8] ~ "ObeseI",
...
var_beta_all_cause_meta = readRDS("var_beta_cancer_meta_pooled.rds")
adjustment_factor = max(diag(Sigma[2:6,2:6])/var_beta_all_cause_meta)
var_external_omega = Sigma[2:6,2:6]/adjustment_factor
diag(var_external_omega) = var_beta_all_cause_meta
A = 1 + sum((exp(fit_unadjusted$coefficients[2:6]) - 1)*prop_exposure[-1])
