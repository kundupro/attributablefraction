...
fit_minimal = svyglm(mortstat_rec ~ bmi_cat + sex + age, design=nhis_samadult_design, family = quasibinomial())
fit_full = svyglm(mortstat_rec ~ bmi_cat + sex + age + smoking_status + education + alcohol_status + marital_status, design=nhis_samadult_design, family = quasibinomial())
...
hh = model.matrix(~ bmi_cat + sex + age + smoking_status + education + alcohol_status + marital_status, data = data_processed_complete)
hh = hh[,-10]
hh[, c(2:6)] = 0
PAR_fully_adjusted = 1 - wtd.mean(exp(hh %*% as.numeric(fit_full$coefficients))/(1 + exp(hh %*% as.numeric(fit_full$coefficients))), data_processed_complete$sampweight)/p_hat
#[1] 8.51002e-05
