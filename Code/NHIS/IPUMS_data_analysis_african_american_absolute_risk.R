...
data_processed$bmi_severe_obese = if_else(data_processed$bmi >= 40, 1, 0)
...
for(j in 1:length(bmi_cat_names))
{
  ...
  fit_full_obese_severe = svyglm(mortstat_rec ~ bmi_severe_obese + sex + age + education + smoking_status + no_of_cigarettes + alcohol_status + marital_status, design=nhis_samadult_design, family = quasibinomial())
  hh = model.matrix(~ bmi_cat + sex + age + education + smoking_status + no_of_cigarettes + alcohol_status + marital_status + bmi_obese + bmi_severe_obese, data = data_processed_complete)
...
#levels(data_plot.m$variable) = c("Observed risk with severe obese BMI category (BMI >= 40)", "Observed risk with obese BMI category (BMI >= 30)", "Observed risk with normal BMI (22-25.7)", "Counterfactual risk")

p1_all_AA = ggplot(data = data_plot.m, aes(x= Decile, y = value)) +   
  ...
  theme_bw() + labs(y="Absolute risk", x="Deciles of risk score", title = "All-cause mortality for the African American population") +
...
colnames(data_plot) = c("Observed risk with severe obese BMI category (BMI >= 40)", "Observed risk with obese BMI category (BMI >= 30)", "Observed risk with normal BMI (22-25.7)", "Counterfactual risk", "Decile")
data_plot.m <- reshape2::melt(data_plot, id.vars='Decile')
p1_heart_AA = ggplot(data_plot.m, aes(Decile, value)) +   
  geom_bar(aes(fill = variable), position = "dodge", stat="identity") +
  scale_fill_manual(values=c("#00A1D5FF","#B24745FF","#79AF97FF", "#6A6599FF"), labels = c(expression(paste("Observed risk for severe obese BMI (","">=40, Kgm^{-2}, ")")), expression(paste("Observed risk for obese BMI (","">=30, Kgm^{-2}, ")")), expression(paste("Observed risk for normal BMI (22-25.7", Kgm^{-2}, ")")), "Counterfactual risk for normal BMI")) +
  theme_bw() + labs(y="Absolute risk", x="Deciles of risk score", title = "Heart disease mortality for the African American population") +
  theme(axis.text.x = element_text(color = "black", size = 15, face="bold"),
        axis.text.y = element_text(color = "black", size = 15, face="bold"),  
        axis.title.x = element_text(color = "black", size = 15, face="bold"),
        axis.title.y = element_text(color = "black", size = 15, face="bold"),
...
colnames(data_plot) = c("Observed risk with severe obese BMI category (BMI >= 40)", "Observed risk with obese BMI category (BMI >= 30)", "Observed risk with normal BMI (22-25.7)", "Counterfactual risk", "Decile")
data_plot.m <- reshape2::melt(data_plot, id.vars='Decile')
p1_cancer_AA = ggplot(data_plot.m, aes(Decile, value)) +   
  geom_bar(aes(fill = variable), position = "dodge", stat="identity") +
  scale_fill_manual(values=c("#00A1D5FF","#B24745FF","#79AF97FF", "#6A6599FF"), labels = c(expression(paste("Observed risk for severe obese BMI (","">=40, Kgm^{-2}, ")")), expression(paste("Observed risk for obese BMI (","">=30, Kgm^{-2}, ")")), expression(paste("Observed risk for normal BMI (22-25.7", Kgm^{-2}, ")")), "Counterfactual risk for normal BMI")) +
  theme_bw() + labs(y="Absolute risk", x="Deciles of risk score", title = "Cancer mortality for the African American population") +
  theme(axis.text.x = element_text(color = "black", size = 15, face="bold"),
        axis.text.y = element_text(color = "black", size = 15, face="bold"),  
        axis.title.x = element_text(color = "black", size = 15, face="bold"),
        axis.title.y = element_text(color = "black", size = 15, face="bold"),
...
p1 = ggarrange(p1_all_white,p1_all_AA,p1_heart_white,p1_heart_AA,p1_cancer_white,p1_cancer_AA,ncol=2,nrow=3)
