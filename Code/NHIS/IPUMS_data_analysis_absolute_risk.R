...
p1_all_white = ggplot(data_plot.m, aes(Decile, value)) +   
  ...
  theme_bw() + labs(y="Absolute risk", x="Deciles of risk score", title = "All-cause mortality for the White population") +
...
fit_full = svyglm(mortstat_rec ~ bmi_cat + sex + age + education + no_of_cigarettes + alcohol_status + marital_status, design=nhis_samadult_design, family = quasibinomial())
...
hh = model.matrix(~ bmi_cat + sex + age + education + no_of_cigarettes + alcohol_status + marital_status, data = data_processed_complete)
hh[, c(2:6)] = 0
...
p1_heart_white = ggplot(data_plot.m, aes(Decile, value)) +   
  geom_bar(aes(fill = variable), position = "dodge", stat="identity") +
  scale_fill_manual(values=c("#00A1D5FF","#B24745FF","#79AF97FF", "#6A6599FF"), labels = c(expression(paste("Observed risk for severe obese BMI (","">=40, Kgm^{-2}, ")")), expression(paste("Observed risk for obese BMI (","">=30, Kgm^{-2}, ")")), expression(paste("Observed risk for normal BMI (22-25.7", Kgm^{-2}, ")")), "Counterfactual risk for normal BMI")) +
  theme_bw() + labs(y="Absolute risk", x="Deciles of risk score", title = "Heart disease mortality for the White population") +
  theme(axis.text.x = element_text(color = "black", size = 15, face="bold"),
        axis.text.y = element_text(color = "black", size = 15, face="bold"),  
        axis.title.x = element_text(color = "black", size = 15, face="bold"),
        axis.title.y = element_text(color = "black", size = 15, face="bold"),
...
p1_cancer_white = ggplot(data_plot.m, aes(Decile, value)) +   
  geom_bar(aes(fill = variable), position = "dodge", stat="identity") +
  scale_fill_manual(values=c("#00A1D5FF","#B24745FF","#79AF97FF", "#6A6599FF"), labels = c(expression(paste("Observed risk for severe obese BMI (","">=40, Kgm^{-2}, ")")), expression(paste("Observed risk for obese BMI (","">=30, Kgm^{-2}, ")")), expression(paste("Observed risk for normal BMI (22-25.7", Kgm^{-2}, ")")), "Counterfactual risk for normal BMI")) +
  theme_bw() + labs(y="Absolute risk", x="Deciles of risk score", title = "Cancer mortality for the White population") +
  theme(axis.text.x = element_text(color = "black", size = 15, face="bold"),
        axis.text.y = element_text(color = "black", size = 15, face="bold"),  
        axis.title.x = element_text(color = "black", size = 15, face="bold"),
        axis.title.y = element_text(color = "black", size = 15, face="bold"),
