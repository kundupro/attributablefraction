...
#---Keeping african_american individuals---#
nhis_ipums_2000_2005  = nhis_ipums_2000_2005[which(nhis_ipums_2000_2005$HISPYN == 2), ]
...
data_processed = nhis_ipums_2000_2005 %>% dplyr::select("mortstat_rec", "bmi_cat", "age", "sex", "education", "smoking_status",
                                                        "strata", "psu", "sampweight", "alcohol_status", "marital_status", "age_at_exit", "year", "no_of_cigarettes", "bmi")  


nrow(na.omit(data_processed[,-12]))
#[1] 107362
data_processed$bmi_obese = if_else(data_processed$bmi >= 30, 1, 0)
...
for(j in 1:length(bmi_cat_names))
{
  ...
  fit_full_obese = svyglm(mortstat_rec ~ bmi_obese + sex + age + education + smoking_status + no_of_cigarettes + alcohol_status + marital_status, design=nhis_samadult_design, family = quasibinomial())
  hh = model.matrix(~ bmi_cat + sex + age + education + smoking_status + no_of_cigarettes + alcohol_status + marital_status + bmi_obese, data = data_processed_complete)
  ...
  hh_risk_score = as.data.frame(hh) %>% mutate(risk_score = as.numeric(hh[, -c(2:6,20:21)] %*% fit_full$coefficients[-c(2:6)]))
  ...
  for(i in 1:10)
  {
    ...
    temp3[, 2] = 1
    AR_fully_adjusted_observed_model[j,i] = wtd.mean(exp(as.matrix(temp[, -c(20:23)]) %*% as.numeric(fit_full$coefficients))/(1 + exp(as.matrix(temp[, -c(20:23)]) %*% as.numeric(fit_full$coefficients))), temp$sampweight)
...
data_plot = as.data.frame(cbind(AR_fully_adjusted_observed_model_with_e0, AR_fully_adjusted_observed_model_with_e0_obese, AR_C_fully_adjusted))
data_plot$decile = rep(paste0(seq(1,10,1)), 1)
data_plot$decile <- factor(data_plot$decile, levels=paste0(seq(1,10,1)))
colnames(data_plot) = c("Observed risk with normal BMI (22-25.7)","Observed risk with obese BMI category (BMI >= 30) ", "Counterfactual risk", "Decile")
data_plot.m <- reshape2::melt(data_plot, id.vars='Decile')
p1_all_hispanic = ggplot(data_plot.m, aes(Decile, value)) +   
  geom_bar(aes(fill = variable), position = "dodge", stat="identity") +
  scale_fill_manual(values=c("#00A1D5FF","#B24745FF","#79AF97FF"), labels = c(expression(paste("Observed risk for normal BMI (22-25.7", Kgm^{-2}, ")")), expression(paste("Observed risk for obese BMI (","">=30, Kgm^{-2}, ")")), "Counterfactual risk for normal BMI")) +
  theme_bw() + labs(y="Absolute risk", x="Deciles of risk score", title = "All-cause mortality for the Hispanic population") +
...
fit_full = svyglm(mortstat_rec ~ bmi_cat + sex + age + no_of_cigarettes + education + alcohol_status + marital_status, design=nhis_samadult_design, family = quasibinomial())
...
hh = model.matrix(~ bmi_cat + sex + age + no_of_cigarettes + education + alcohol_status + marital_status, data = data_processed_complete)
hh[, c(2:6)] = 0
...
colnames(data_plot) = c("Observed risk with normal BMI (22-25.7)","Observed risk with obese BMI category (BMI >= 30) ", "Counterfactual risk", "Decile")
data_plot.m <- reshape2::melt(data_plot, id.vars='Decile')
p1_heart_hispanic = ggplot(data_plot.m, aes(Decile, value)) +   
  geom_bar(aes(fill = variable), position = "dodge", stat="identity") +
  scale_fill_manual(values=c("#00A1D5FF","#B24745FF","#79AF97FF"), labels = c(expression(paste("Observed risk for normal BMI (22-25.7", Kgm^{-2}, ")")), expression(paste("Observed risk for obese BMI (","">=30, Kgm^{-2}, ")")), "Counterfactual risk for normal BMI")) +
  theme_bw() + labs(y="Absolute risk", x="Deciles of risk score", title = "Heart disease mortality for the Hispanic population") +
  theme(axis.text.x = element_text(color = "black", size = 15, face="bold"),
        axis.text.y = element_text(color = "black", size = 15, face="bold"),  
        axis.title.x = element_text(color = "black", size = 15, face="bold"),
        axis.title.y = element_text(color = "black", size = 15, face="bold"),
...
colnames(data_plot) = c("Observed risk with normal BMI (22-25.7)","Observed risk with obese BMI category (BMI >= 30) ", "Counterfactual risk", "Decile")
data_plot.m <- reshape2::melt(data_plot, id.vars='Decile')
p1_cancer_hispanic = ggplot(data_plot.m, aes(Decile, value)) +   
  geom_bar(aes(fill = variable), position = "dodge", stat="identity") +
  scale_fill_manual(values=c("#00A1D5FF","#B24745FF","#79AF97FF"), labels = c(expression(paste("Observed risk for normal BMI (22-25.7", Kgm^{-2}, ")")), expression(paste("Observed risk for obese BMI (","">=30, Kgm^{-2}, ")")), "Counterfactual risk for normal BMI")) +
  theme_bw() + labs(y="Absolute risk", x="Deciles of risk score", title = "Cancer mortality for the Hispanic population") +
  theme(axis.text.x = element_text(color = "black", size = 15, face="bold"),
        axis.text.y = element_text(color = "black", size = 15, face="bold"),  
        axis.title.x = element_text(color = "black", size = 15, face="bold"),
        axis.title.y = element_text(color = "black", size = 15, face="bold"),
...
p1 = ggarrange(p1_all_white,p1_all_AA,p1_all_hispanic, p1_heart_white,p1_heart_AA, p1_heart_hispanic, p1_cancer_white,p1_cancer_AA, p1_cancer_hispanic, ncol=3,nrow=3)
ggsave('/Users/prosenjitkundu/Dropbox/Attributable_risk/Absolute_and_attributable_risk/Plots/absolute_risk.png', p1, width = 17,height=11, dpi = 300)
